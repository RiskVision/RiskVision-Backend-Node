...
router.get('/', async (req, res) => {
    try {
        ...
        const result = await pool.request()
            .query(`
                SELECT * FROM dbo.asset_Inventory
            `);
...
router.delete('/:id_activo', async (req, res) => {
    try {
        const pool = await poolPromise;
        const { id_activo } = req.params;
        await pool.request().input('id_activo', sql.VarChar, id_activo).query('DELETE FROM dbo.asset_Inventory WHERE id_activo = @id_activo');
